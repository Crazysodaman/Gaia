    ...
    while True:
        ...
        if (IMU.LSM9DS0):
            magYcomp = MAGx * math.sin(roll) * math.sin(pitch) + MAGy * math.cos(roll) - MAGz * math.sin(
                roll) * math.cos(pitch)  # LSM9DS0
        else:
            magYcomp = MAGx * math.sin(roll) * math.sin(pitch) + MAGy * math.cos(roll) + MAGz * math.sin(
                roll) * math.cos(pitch)  # LSM9DS1
        ...
        data.ACCx_change(ACCx)
        data.ACCz_change(ACCz)
        data.ACCy_change(ACCy)
        data.GYRx_change(GYRx)
        data.GYRy_change(GYRy)
        data.GYRz_change(GYRz)
        data.MAGz_change(MAGz)
        data.MAGx_change(MAGx)
        data.MAGy_change(MAGy)
        data.kalmanX_change(kalmanX)
        data.kalmanY_change(kalmanY)
        data.tiltCompensatedHeading_change(tiltCompensatedHeading)
